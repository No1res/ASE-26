...
class RoleScore:
    ...
    matches: List[str] = field(default_factory=list)  # 记录命中的规则/模式
...
class CodeRoleClassifier:
    """
    RAACS 3.1: Hybrid Code Role Classifier
    结合 Explicit Framework Fingerprints (强规则) 与 Implicit Structural Patterns (泛化模式)
    """
    
    # --- 1. 显式框架指纹 (Strong Signals) ---
    ...
    def _extract_features(self, tree: ast.AST, file_path: str) -> Dict[str, Any]:
        ...
        for node in ast.walk(tree):
            ...
            # Functions
                ...
                f['function_count'] += 1
                if node.name.startswith('test_'): f['sig_test'] += 1
    ...
    def _analyze_structural_pattern(self, tree: ast.AST, features: Dict) -> StructuralPattern:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                # IO Boundary (Semantic Check)
                if self._is_io_boundary_func(node): io_funcs += 1
                
                # Orchestrator (Structure Check)
                ...
                # Data Class Heuristic: Fields >> Methods
        ...
        # Calculate Ratios using Config Thresholds
        if total_funcs > 0:
        ...
        # IO Boundary Logic: 
        # Hybrid decision: If Explicit Signal OR Semantic Pattern
    ...
    def _calculate_hybrid_score(self, p: StructuralPattern, f: Dict, file_path: str) -> RoleScore:
        ...
        # --- 1. Framework Signals (强权重) ---
        if f['sig_test'] > 0:
            add_score(Role.TEST, self.config.WEIGHT_FRAMEWORK, "Framework: Test imports/classes")
        ...
        # --- 2. Structural Patterns (泛化权重) ---
        
        # TEST Patterns
        if p.assertion_density > 0.5:
            add_score(Role.TEST, self.config.WEIGHT_PATTERN, f"Pattern: High assertion density ({p.assertion_density:.1f})")
        ...
        if p.data_definition_ratio > self.config.DATA_RATIO_THRESHOLD:
            ...
            add_score(Role.SCHEMA, 1.5, "Pattern: Moderate data definitions with low logic")
        ...
        if p.is_io_boundary and f['sig_web_adapter'] == 0:
            # 只有在没有显式框架信号时，才主要依赖 IO Pattern 加分，避免重复加分
            add_score(Role.ADAPTER, self.config.WEIGHT_PATTERN, "Pattern: IO Boundary detected (naming/types)")
    ...
    def _is_io_boundary_func(self, node: ast.FunctionDef) -> bool:
        """
        IO检测：混合了命名启发式(Heuristic)和结构特征
        """
        # 1. 命名检测 (Configurable Keywords)
        param_names = {arg.arg.lower() for arg in node.args.args}
        if not param_names.isdisjoint(self.config.IO_KEYWORDS):
            return True
            
        # 2. 返回值检测
        if node.returns:
            ret_name = self._get_name(node.returns).lower()
            if any(k in ret_name for k in ['response', 'json', 'dict', 'schema']):
                return True
...
if __name__ == "__main__":
    ...
    if os.path.isfile(target):
        ...
        if args.detail:
            ...
            if r.pattern:
                print(f"Pattern: IO={r.pattern.is_io_boundary}, DataRatio={r.pattern.data_definition_ratio:.2f}")
    else:
        print("-" * 80)
